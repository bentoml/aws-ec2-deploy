...
def update_deployment(bento_bundle_path, deployment_name, config_json):
    ...
    template_name, stack_name, s3_bucket_name, repo_name, elb_name = generate_ec2_resource_names(deployment_name)
    ...
    repository_id, registry_url = create_ecr_repository_if_not_exists(ec2_config['region'], repo_name)
    ...
    project_path = os.path.join(
        os.path.curdir,
        f'{bento_metadata.name}-{bento_metadata.version}-deployable'
    )
    ...
    generate_cloudformation_template_file(
        ...
        health_check_interval_seconds=ec2_config['elb']['health_check_interval_seconds'],
    ...
    run_shell_command(
        command=['sam', 'package', '--output-template-file', 'packaged.yaml', '--s3-bucket', s3_bucket_name],
        cwd=project_path,
        env=copied_env,
    )
    print('Deploy EC2')
